...
if __name__ == "__main__":
    ...
    while True:
        ...
        if actionNum == 1:
            clientLibrary.fileLists(clientLibrary, address, local_catch)

        elif actionNum == 2:
            fileToOpen = input("Input the complete filename you want to open: ")
            clientLibrary.readFile(clientLibrary, address, fileToOpen, local_catch)
            ...
            clientLibrary.addFile(clientLibrary, address, fileToAdd, fileData, local_catch)

        elif actionNum == 4:
            fileToAdd = input("Input the complete filename you want to edit: ")
            fileData = input("Input the new data in {}: ".format(fileToAdd))
            clientLibrary.editFile(clientLibrary, address, fileToAdd, fileData, local_catch)
            ...
            if yn == "n" or yn == "N":
                continue
            clientLibrary.deleteFile(clientLibrary, address, fileToDelete, local_catch)

        elif actionNum == 6:
            fileToPush = input("Input the complete filename you want to push: ")
            clientLibrary.pushFile(clientLibrary, address, fileToPush, local_catch)
